...
\institute{Prof. Sana Stojanović Đurđević}
...
\begin{itemize}
    \item Primarni ključ: jednoznačno identifikuje svaki slog.
    \item Može biti prost (jedan atribut) ili složen (više atributa).
    \item Sekundarni ključevi (indeksi) omogućavaju bržu pretragu i sortiranje.
    \item Veštački ključevi
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Primena pravila unosa, podrazumevanih vrednosti i maskiranja podataka.
...
\bigskip
\begin{itemize}
    \item Primarni ključ jedne tabele povezuje se sa stranim ključem druge tabele
    \item Referencijalni integritet garantuje doslednost podataka
    \item Svaka vrednost stranog ključa mora postojati kao primarni ključ ili može biti prazna
\end{itemize}
...
\begin{center}
\textbf{Veze se mogu menjati dodavanjem, uklanjanjem ili modifikovanjem:}
\bigskip
\begin{itemize}
...
\bigskip
\textit{Migracija podataka ili privremena prilagodba strukture može biti potrebna za očuvanje stabilnosti sistema.}
\end{center}
\end{frame}

\begin{frame}{Forme (obrasci)}
\begin{center}
\textbf{Forme omogućavaju lakši i pregledniji unos i prikaz podataka u bazi.}
...
\begin{center}
\textbf{Forme služe za unos podataka u bazu bez direktnog rada sa tabelom.}
...
\begin{center}
\textbf{Specijalne kontrole olakšavaju rad i navigaciju u bazi:}
...
\begin{center}
\textbf{Omogućavaju pregled i unos podataka iz više tabela ili upita istovremeno.}

\bigskip
\begin{itemize}
    \item Pogodne za prikaz povezanih podataka (npr. proizvodi i količina po magacinima)
...
\begin{center}
\textbf{Pretraživanje omogućava brzo pronalaženje željenih slogova.}
...
\begin{center}
\textbf{Sortiranje i filtriranje poboljšavaju preglednost i analizu podataka.}
...
\begin{frame}{Upiti u bazama podataka}
\begin{center}
\textbf{Upiti omogućavaju pregled, dodavanje, izmenu i brisanje podataka.}

\bigskip
\begin{itemize}
    \item Komunikacija sa bazom preko \textit{SQL} jezika
    \item Omogućava kreiranje baze i tabela, upisivanje, menjanje, pretraživanje i brisanje podataka
\end{itemize}
\end{center}
\end{frame}
...
\begin{center}
\textbf{\textit{SQL} komande se dele u tri osnovne grupe:}
...
\begin{center}
\textbf{Kontrolne naredbe upravljaju pravima pristupa korisnika:}
\bigskip
\begin{itemize}
...
\begin{center}
\textbf{Definicione naredbe služe za rukovanje objektima baze:}
\bigskip
\begin{itemize}
...
\begin{center}
\textbf{Manipulativne naredbe su najčešće korišćene:}
\bigskip
\begin{itemize}
...
\begin{center}
\textbf{Upiti se mogu kreirati na dva načina:}
\bigskip
\begin{itemize}
...
\end{itemize}
\bigskip
\textit{Pre kreiranja upita važno je definisati zadatak i podatke koje želimo prikazati.}
\end{center}
\end{frame}

\begin{frame}{Pregled rezultata upita}
\begin{center}
\textbf{Rezultati \textit{SQL} upita prikazuju se u tabelarnom obliku:}
\bigskip
\begin{itemize}
...
\begin{frame}{Pretraživanje i \textit{SELECT} upiti}
\begin{itemize}
    \item \textit{SELECT} - čitanje podataka iz baze
    \item \textit{INSERT} - unos novih podataka
    \item \textit{UPDATE} - izmena postojećih podataka
    \item \textit{DELETE} - brisanje podataka
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Preporučljivo prvo dodati i rasporediti povezane kontrole (\textit{bound})
...
\begin{itemize}
    ...
    \item Upotreba podizveštaja (\textit{subreports}) za dodatne detalje
    ...
    \end{itemize}
    \item Posebno korisno za višedimenzionalne informacije:
    \begin{itemize}
        \item Lageri po magacinima
        \item Prodaja po proizvodima
        \item Finansijski pokazatelji po odeljenjima
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Poboljšava korisničko iskustvo i olakšava rad sa podacima
\end{itemize}
\end{frame}
